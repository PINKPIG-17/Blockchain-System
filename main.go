...
func main() {
	...
	maker := maker2.NewBlockMaker(&txpol, &Stadb, &machine, &chain, datedb)
	...
	for i := 0; i < 10; i++ {
		tx := types.NewTx(&alice, addr_bob, uint64(i+1), 1, 2000, 1, []byte("Send 1 eth."))
		txpol.NewTx(tx)
		tx = types.NewTx(&alice, addr_bob, uint64(i+2), 1, 2000, 1, []byte("Send 1 eth."))
		txpol.NewTx(tx)

	}

	for i := 0; i < 10; i++ {
		tx := types.NewTx(&bob, addr_alice, uint64(i+1), 2, 2000, 1, []byte("Send 2 eth."))
		txpol.NewTx(tx)
		tx = types.NewTx(&bob, addr_alice, uint64(i+2), 2, 2000, 1, []byte("Send 2 eth."))
		txpol.NewTx(tx)
	}
	maker.Seal()
}
